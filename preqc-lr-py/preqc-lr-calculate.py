...
def calculate_est_cov(fasta, output_prefix, data):
	...
	print "Num reads: " + str(n)
	print "Mean read len: "  + str(l)
	print "Median cov:  " + str(c)
	...
	with open(overlaps_filename, "r") as overlaps:
		for line in overlaps:
			...
			if not (query_read_id == target_read_id):
				...
				sum_clips += left_clip + right_clip
		...
		sum_tot_len_ol=0
		sum_tot_ol=0
		for i in paf_records.values():
			print i.read_id + ": " + str(i.read_length) + "," + str(i.total_num_overlaps) + "," + str(i.total_len_overlaps) 
		print "TOTAL OL LENGTH ACROSS ALL READS: " + str(sum_tot_len_ol) 
		print "TOTAL OL NUMBER ACROSS ALL READS: " + str(sum_tot_ol)
#		print sum_clips
#		print "sum ols without clips: " + str(sum_ol)
